...
import ConnectionPoolDiagram from "@/components/blog/ConnectionPoolDiagram";
...
strip = true          # no debug symbols in the shipped binary`}
          ...
          <H3>The mistake that was actually costing the most time: rebuilding the client every Send</H3>
          <P>
            Startup speed and binary size get all the attention, but they weren&apos;t the
            biggest lever. For a long time, <span className="font-mono text-sm text-neutral-400">execute_request</span> called{" "}
            <span className="font-mono text-sm text-neutral-400">build_client(&amp;request)</span> on
            every single Send — a brand-new <span className="font-mono text-sm text-neutral-400">reqwest::Client</span> from
            scratch, every time. That sounds harmless until you remember what a{" "}
            <span className="font-mono text-sm text-neutral-400">Client</span> actually is: it&apos;s
            not just config, it&apos;s the thing holding an open, warm TCP/TLS connection and a
            DNS cache. Throwing it away after one request meant every repeat click to the same
            API paid a full DNS lookup + TCP handshake + TLS handshake again — often more time
            than the actual request/response itself.
          </P>

          <ConnectionPoolDiagram />

          <P>
            The fix isn&apos;t an algorithm, it&apos;s a cache: a small{" "}
            <span className="font-mono text-sm text-neutral-400">HashMap</span> keyed by the
            handful of settings that actually change how a client is built (timeout, redirect
            policy, SSL verification, cookie jar). Two requests with identical settings —
            which is most requests, most of the time — now share one long-lived client instead
            of each getting a disposable one.
          </P>
          <CodeBlock title="src-tauri/src/commands/http.rs" lang="rust">
...
impl ClientPool {
    async fn get_or_build(&self, request: &ApiRequest) -> Result<Arc<reqwest::Client>, String> {
        ...
        if let Some(client) = self.0.lock().await.get(&key) {
            return Ok(client.clone()); // same warm connection, no network work at all
        }
        let client = Arc::new(build_client(request)?);
        self.0.lock().await.insert(key, client.clone());
        Ok(client)
    }
}`}
          </CodeBlock>
          <P>
            One side effect worth calling out: this also means <span className="font-mono text-sm text-neutral-400">save_cookies</span> now
            actually behaves the way you&apos;d expect. With a fresh client per request, a
            login-then-use-the-session-cookie flow silently never worked — every request
            started from an empty jar. Reusing the client means the cookie jar persists across
            Sends to the same host, same as a real browser.
          </P>
          <P>
            HTTP/2 multiplexing is the other quiet beneficiary here.{" "}
            <span className="font-mono text-sm text-neutral-400">reqwest</span> negotiates it
            automatically over TLS — but only on a connection that lives long enough to prove
            it. A connection that gets torn down after one request never gets the chance.
            Pooling the client is what actually lets that negotiation pay off.
          </P>
