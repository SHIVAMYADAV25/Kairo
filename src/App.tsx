        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          {activeTab ? (
            <>
              ...
              {responseOpen && (
                <>
                  <div style={{ height: settings.panelSizes.responseViewerHeight }} className="shrink-0 overflow-hidden">
                    <ResponseViewer tab={activeTab} onClose={() => setResponseOpen(false)} />
                  </div>
                  <ResizeHandle
                    direction="vertical"
                    onResize={(d) =>
                      update({ panelSizes: { ...settings.panelSizes, responseViewerHeight: Math.max(160, settings.panelSizes.responseViewerHeight + d) } })
                    }
                  />
                </>
              )}
