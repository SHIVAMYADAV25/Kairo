...
/**
 * Classifies a raw error string (from the Rust/reqwest layer, e.g.
 * "error sending request for url (...): error trying to connect: tcp
 * connect error: Connection refused (os error 111)") into a short
 * category label + icon for scannability.
 *
 * Unlike the old version, this never rewrites or hides the actual
 * error text — the raw message from reqwest/hyper/rustls is always
 * shown as-is, the same way Postman, Bruno, Insomnia, and Hoppscotch
 * surface driver errors directly instead of paraphrasing them.
 */
export type ErrorIcon = "network" | "timeout" | "ssl" | "url" | "refused" | "generic";

export function classifyError(raw: string | null | undefined): ErrorIcon {
  const err = (raw ?? "").toLowerCase();

  if (!raw) return "generic";
  if (err.includes("dns") || err.includes("lookup address") || err.includes("nodename")) return "url";
  if (err.includes("connection refused") || err.includes("refused")) return "refused";
  if (err.includes("timed out") || err.includes("timeout")) return "timeout";
  if (err.includes("certificate") || err.includes("ssl") || err.includes("tls")) return "ssl";
  if (err.includes("invalid url") || err.includes("relative url") || err.includes("empty url") || err.includes("builder error")) return "url";
  if (err.includes("connection reset") || err.includes("broken pipe") || err.includes("connection closed")) return "network";
  if (err.includes("too many redirects")) return "generic";
  if (err.includes("network is unreachable") || err.includes("could not connect")) return "network";
