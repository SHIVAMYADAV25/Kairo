...
function descendantIds(collections: Collection[], parentId: string): string[] {
  const direct = collections.filter((c) => c.parentId === parentId).map((c) => c.id);
  return direct.flatMap((id) => [id, ...descendantIds(collections, id)]);
}
...
interface CollectionState {
  ...
  renameCollection: (id: string, name: string) => Promise<void>;
  deleteCollection: (id: string) => Promise<void>;
  /** Drag-reorder: moves a collection to be a child of `newParentId`
   * (null = root). Guards against dropping a collection into its own
   * descendant. */
  reorderCollectionInto: (id: string, newParentId: string | null) => Promise<void>;
  ...
  renameRequest: (request: ApiRequest, name: string) => Promise<void>;
  deleteRequest: (collectionId: string, requestId: string) => Promise<void>;
  /** Drag-reorder: moves a request into a different collection. */
  moveRequestToCollection: (request: ApiRequest, newCollectionId: string) => Promise<void>;
  /** Gathers every request in a collection + all its nested subfolders —
   * used by the Collection Runner. */
  collectRequestsRecursive: (collectionId: string) => Promise<ApiRequest[]>;
...
export const useCollectionStore = create<CollectionState>((set, get) => ({
  ...
  reorderCollectionInto: async (id, newParentId) => {
    if (id === newParentId) return;

    const isDescendant = (candidateId: string, ancestorId: string): boolean => {
      const c = get().collections.find((c) => c.id === candidateId);
      if (!c || c.parentId === null) return false;
      if (c.parentId === ancestorId) return true;
      return isDescendant(c.parentId, ancestorId);
    };
    if (newParentId && isDescendant(newParentId, id)) return; // would create a cycle

    const siblingCount = get().collections.filter((c) => c.parentId === newParentId).length;
    await api.collections.reorder(id, newParentId, siblingCount);
    set((s) => ({
      collections: s.collections.map((c) => (c.id === id ? { ...c, parentId: newParentId, position: siblingCount } : c)),
  ...
  upsertRequestInCache: (saved) => {
    ...
    set((s) => {
      ...
      const exists = list.some((r) => r.id === saved.id);
      const nextList = exists ? list.map((r) => (r.id === saved.id ? saved : r)) : [...list, saved];
      return { requestsByCollection: { ...s.requestsByCollection, [saved.collectionId!]: nextList } };
    });
  },
  ...
  moveRequestToCollection: async (request, newCollectionId) => {
    if (request.collectionId === newCollectionId) return;
    const oldCollectionId = request.collectionId;
    const saved = await api.requests.save({ ...request, collectionId: newCollectionId });
    set((s) => {
      const requestsByCollection = { ...s.requestsByCollection };
      if (oldCollectionId) {
        requestsByCollection[oldCollectionId] = (requestsByCollection[oldCollectionId] ?? []).filter((r) => r.id !== request.id);
      }
      requestsByCollection[newCollectionId] = [...(requestsByCollection[newCollectionId] ?? []), saved];
      return { requestsByCollection, expanded: { ...s.expanded, [newCollectionId]: true } };
    });
    useTabStore.getState().syncSavedRequest(saved.id, saved);
  },

  collectRequestsRecursive: async (collectionId) => {
    const ids = [collectionId, ...descendantIds(get().collections, collectionId)];
    const all: ApiRequest[] = [];
    for (const id of ids) {
      await get().ensureRequestsLoaded(id);
      all.push(...(get().requestsByCollection[id] ?? []));
    }
    return all;
  },
